...
namespace FluffyUnderware.Curvy.Utils
{
    ...
    public class Spline2Mesh
    {
        ...
        void uvmap()
        {
            ...
            mMesh.uv = uv;
            mMesh.uv2 = UV2 ? new Vector2[uv.Length] : new Vector2[0];
            mMesh.RecalculateNormals();
        }
        ...
        public Vector3[] getVertices()
        {
            switch (VertexMode) {
                case VertexCalculation.ByAngle:
                    Vector3[] points = Spline.GetPolygonByAngle(Angle, Distance);
                    return points;
                default:
    ...
    {
        /// <summary>
        /// Gets the DeltaTime - even in the editor
        /// </summary>
        public static float DeltaTime
        {
            get
            {
                return (Application.isPlaying) ? Time.deltaTime : mEditorDeltaTime;
            }
        }
        static float mEditorDeltaTime;
        static float mEditorLastTime;
        static int mEditorLastFrame;

        /// <summary>
        /// Called frequently to calculate DeltaTime in the editor
        /// </summary>
        public static void SetEditorTiming()
        {
            int frameDelta = Time.frameCount - mEditorLastFrame;
            float cur = Time.realtimeSinceStartup;
            float timeDelta = (cur - mEditorLastTime);
            ...
            {
                mEditorDeltaTime = timeDelta / frameDelta;
                mEditorLastTime = cur;
                mEditorLastFrame = Time.frameCount;
            }
        }

        [System.Obsolete("Use IsPlanar() instead")]
        public static bool isPlanar(CurvySpline spline, out int ignoreAxis) { return IsPlanar(spline, out ignoreAxis); }
        [System.Obsolete("Use IsPlanar() instead")]
        public static bool isPlanar(CurvySpline spline, out bool xplanar, out bool yplanar, out bool zplanar) { return IsPlanar(spline, out xplanar, out yplanar, out zplanar); }
        [System.Obsolete("Use MakePlanar() instead")]
        public static void makePlanar(CurvySpline spline, int axis) { MakePlanar(spline, axis); }
        [System.Obsolete("Use CenterPivot() instead")]
        public static void centerPivot(CurvySpline spline) { CenterPivot(spline); }
        [System.Obsolete("Use SetFirstCP() instead")]
        public static void setFirstCP(CurvySplineSegment newStartCP) { SetFirstCP(newStartCP); }
        /// <summary>
